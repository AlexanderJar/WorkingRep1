...
JM = JointMod(meshRST, ert, rst, fpm, ntimes=ntimes, fix_poro=fix_poro, zWeight=0.25)
...
# 2D L-Curve: try different lambdas and gammas

chis = np.zeros((len(lams),len(gams)))
rmss = np.zeros((len(lams),len(gams)))
...
for lam in lams:
    ...
    for gam in gams:
        ...
        inv = JointInv(JM, data, error, startmodel, lam=lam, gamma=gam, frmin=fr_min,
        ...
        if timelapse == True:
            np.savez("TL_joint_inversion_%s_Lcurve_lam%s_gam%s.npz" % (scenario,lam,gam), vel=np.array(velest),
