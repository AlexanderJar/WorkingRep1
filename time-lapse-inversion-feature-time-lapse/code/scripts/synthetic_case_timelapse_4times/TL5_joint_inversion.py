...
ertScheme4 = pg.DataContainerERT("erttrue_t4.dat")
...
ttData4 = pg.DataContainer("tttrue_t4.dat", "s g")
...
if timelapse == True:
    ...
    if timestep == 1:
        ...
        tts = ttData4("t")
        rhoas = ertScheme4("rhoa")
        errors_tt = ttData4("err") / ttData4("t")
        errors_rhoa = ertScheme4("err")
...
startmodel3 = createGradientModel2D(ttData3, meshRST, np.min(veltrue[mesh.cellCount():]),
                                np.max(veltrue[mesh.cellCount():]))                  
startmodel4 = createGradientModel2D(ttData4, meshRST, np.min(veltrue[mesh.cellCount():]),
                                np.max(veltrue[mesh.cellCount():]))                       
if timelapse == True:
    startmodel = np.concatenate((startmodel, startmodel2))
    startmodel = np.concatenate((startmodel, startmodel3))
    startmodel = np.concatenate((startmodel, startmodel4))
elif timelapse == False:
    if timestep == 2:
...
if timelapse == True:
    ...
    rhostart4 = np.ones_like(velstart[0:meshRST.cellCount()]) * np.mean(ertScheme4("rhoa"))
    rhostart = np.concatenate((rhostart, rhostart4))
elif timelapse == False:
    if timestep == 2:
    ...
    if timestep == 4:
        rhostart = np.ones_like(velstart[0:meshRST.cellCount()]) * np.mean(ertScheme4("rhoa"))
...
if timelapse == False:
    np.savez("TL_joint_inversion_%s_t%s.npz" % (scenario, timestep), vel=np.array(velest),
            rho=np.array(rhoest), fa=fae, fi=fie, fw=fwe, fr=fre, mask=array_mask)
