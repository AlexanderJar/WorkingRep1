...
# Choose: Plot individual inversions or TL inversion?
timelapse = True
...
for timestep in range(1,ntimes+1):
    ...
    meshj = pg.load("paraDomain_TL_%d.bms" % case)
    true = np.load("true_model_TL.npz")
    sensors = np.load("sensors_TL.npy", allow_pickle=True)
    est = np.load("conventional_TL_%s_t%s.npz" % (scenario, timestep))
    if timelapse == True:
        joint = np.load("TL_joint_inversion_%s_lam10.0_gam1_zW1.0_delta100_it4_atc.npz" % scenario)
    elif timelapse == False:
        joint = np.load("TL_joint_inversion_%s_t%s.npz" % (scenario, timestep))
    ...
    if timelapse == True:
        veljoint = veljoint[nCellsj*(timestep-1):nCellsj*timestep]
        rhojoint = rhojoint[nCellsj*(timestep-1):nCellsj*timestep]
        faj = faj[nCellsj*(timestep-1):nCellsj*timestep]
        fij = fij[nCellsj*(timestep-1):nCellsj*timestep]
        fwj = fwj[nCellsj*(timestep-1):nCellsj*timestep]
        frj = frj[nCellsj*(timestep-1):nCellsj*timestep]
        maskj = maskj[nCellsj*(timestep-1):nCellsj*timestep]
    ...
    fre = 1 - fwe - fae - fie
    sume = fae + fie + fwe + fre
    sumj = faj + fij + fwj + frj
    sumtrue = fa + fi + fw + fr
    ...
    def minmax(data):
        ...
        print("max", tmp.max())
        min = tmp.min()
        if np.max(tmp) > 10 and np.max(tmp) < 1e3:
            return "min: %d | max: %d" % (min, tmp.max())
        if np.min(tmp) > 1e3:
            return "min: " + logFormat(tmp.min()) + " | max: " + logFormat(tmp.max())
        if np.max(tmp) > 1e3:
    ...
    grid = ImageGrid(fig, 111, nrows_ncols=(7, 3), axes_pad=[0.03, 0.03],
                    share_all=True, add_all=True, cbar_location="right",
                    cbar_mode="edge", cbar_size="5%", cbar_pad=0.05, aspect=True)

    cov = rst_cov(meshj, np.loadtxt("rst_coverage_TL_%d_t%s.dat" % (case, timestep)))
    ...
    long_labels = [
        "Vel.", "Res.", "Water content", "Ice content", "Air content",
        "Rock content", "Sum"
    ]
    meshs = [mesh, meshj, meshj]
    cmaps = ["viridis", "Spectral_r", "Blues", "Purples", "Greens", "Oranges", "RdBu"]
    datas = [(veltrue, velest, veljoint), (rhotrue, rhoest, rhojoint),
            (fw, fwe, fwj), (fi, fie, fij), (fa, fae, faj), (fr, fre, frj), (sumtrue, sume, sumj)]
    ...
    for i, (row, data, label,
        ...
        if i == 0:
            ...
            lims = {"cMin": 0.0, "cMax": 0.15}
        elif i == 5:
            lims = {"cMin": 0.6, "cMax": 0.8}
        elif i == 6:
            lims = {"cMin": 0.9, "cMax": 1.1}
        else:
            lims = lim(
        ...
        for j, ax in enumerate(row):
            ...
            color = "k" if j == 0 and i not in (1, 5) else "w"
            if i == 3 and j == 0:
                ims.append(
                draw(ax, meshs[j], data[j], cMin=0.1, cMax=0.3, logScale=logScale,
                    coverage=coverage))
            else:
                ims.append(
        ...
        if i == 1:
            update_ticks(cb, log=logScale, label=label, cMin=1e3, cMax=2e5)
        if i == 3:
            update_ticks(cb, log=logScale, label=label, cMin=0.0, cMax=0.3)
        else:
            update_ticks(cb, log=logScale, label=label, **lims)
            

    for ax, title, num in zip(grid.axes_row[0], [
            "True model", "Conv. inversion and 4PM",
    ...
    labs = [
        "Transf.", "Transf.", "Inverted", "Inverted", "Inverted",
        "Inverted", "Calculated"
    ]
    ...
    for i, (ax, label) in enumerate(zip(grid.axes_column[0], long_labels)):
        ...
        color = "k" if i not in (1, 3) else "w"
    ...
    if timelapse == True:
        fig.savefig("TL_%s_t%s_two_columns_with_sum_lam10.0_gam1_zW1.0_delta100_it4_atc.pdf" % (scenario, timestep), dpi=500, bbox_inches="tight",
                    pad_inches=0.0)
    elif timelapse == False:
        fig.savefig("%s_t%s_two_columns_with_sum.pdf" % (scenario, timestep), dpi=500, bbox_inches="tight",
                    pad_inches=0.0)
