...
chis = np.zeros((len(lams),len(epss)))
rmss = np.zeros((len(lams),len(epss)))
...
for lam in lams:
    ...
    for eps in epss:
        ...
        np.savez("TL_joint_inversion_SCH_Lcurve_lam%s_zW%s_delta%s_gam%s_eps%s_it%s_noatc.npz" % (lam,zW,delta,gam,eps,maxIter), vel=np.array(velest),
...
np.save("chi_noatc.npy",chis,allow_pickle=True)
np.save("rms_noatc.npy",rmss,allow_pickle=True)
np.save("epss_noatc.npy",epss,allow_pickle=True)
np.save("lambdas_noatc.npy",lams,allow_pickle=True)
