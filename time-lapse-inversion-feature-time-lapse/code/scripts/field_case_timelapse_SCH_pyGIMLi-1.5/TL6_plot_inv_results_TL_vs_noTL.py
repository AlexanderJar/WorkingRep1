...
sensors = np.loadtxt("Output_file/sensor_file/sensors_t1.npy")
...
for timestep in range(1,ntimes+1):
    ...
    cov = rst_cov(mesh, np.loadtxt("Output_file/conventional_inv/rst_coverage_t%s.dat" % timestep))
    ...
    fig.savefig("Output_file/test_t%s_dwia.pdf" % timestep, dpi=500, bbox_inches="tight",
                pad_inches=0.0)
