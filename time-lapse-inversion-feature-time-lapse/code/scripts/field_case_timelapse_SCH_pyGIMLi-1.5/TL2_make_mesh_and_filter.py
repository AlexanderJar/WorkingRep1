...
try:     #added by Alex 25
    shutil.rmtree('Output_file/data_filtered')     #added by Alex 25
except:     #added by Alex 25
    print('Create data filtered file')     #added by Alex 25
os.mkdir('Output_file/data_filtered')     #added by Alex 25

try:     #added by Alex 25
    shutil.rmtree('Output_file/sensor_file')     #added by Alex 25
except:     #added by Alex 25
    print('Create sensor file')     #added by Alex 25
os.mkdir('Output_file/sensor_file')     #added by Alex 25

try:     #added by Alex 25
    shutil.rmtree('Output_file/mesh_file')     #added by Alex 25
except:     #added by Alex 25
    print('Create mesh file')     #added by Alex 25
os.mkdir('Output_file/mesh_file')     #added by Alex 25
...
for timestep in range(1,ntimes+1):
    ertData.append(pg.DataContainer("data/ert_t%s.data" % timestep))
    print("Number of electrodes t%s:" % timestep, ertData[timestep-1].sensorCount())
    print(ertData[timestep-1])

    rstData.append(pg.DataContainer("data/rst_t%s.data" % timestep, "s g"))
    ...
    ertData[timestep-1].save("Output_file/data_filtered/ert_filtered_t%s.data" % timestep)
    ...
    rstData[timestep-1].save("Output_file/data_filtered/rst_filtered_t%s.data" % timestep)
    ...
    np.savetxt("Output_file/sensor_file/sensors_t%s.npy" % timestep, np.column_stack((x, z)))

    print("Number of combined positions t%s:" % timestep, combinedSensors.sensorCount())
    print(combinedSensors)
# %%

    combinedSensors = np.loadtxt("Output_file/sensor_file/sensors_t1.npy")
    ...
    for case in 1, 2:
        ...
        mesh.save("Output_file/mesh_file/mesh_%s_t%s.bms" % (case, timestep))
        ...
        paraDomain.save("Output_file/mesh_file/paraDomain_%s_t%s.bms" % (case, timestep))
