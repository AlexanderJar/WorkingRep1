...
for timestep in range(1,ntimes+1):
    ...
    if timelapse == True:
        joint = np.load("TL_joint_inversion_%s.npz" % scenario)
    ...
    if timelapse == True:
        fig.savefig("TL_%s_t%s_two_columns_with_sum.pdf" % (scenario, timestep), dpi=500, bbox_inches="tight",
                    pad_inches=0.0)
    elif timelapse == False:
