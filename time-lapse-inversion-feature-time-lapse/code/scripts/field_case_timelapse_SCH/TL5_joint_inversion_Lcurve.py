...
for lam in lams:
    ...
    for delta in deltas:
        ...
        np.savez("Lcurve/TL_joint_inversion_SCH_Lcurve_lam%s_zW%s_delta%s_gam%s_eps%s_it%s.npz" % (lam,zW,delta,gam,eps,maxIter), vel=np.array(velest),
...
np.save("deltas_Lcurve.npy",deltas,allow_pickle=True)
