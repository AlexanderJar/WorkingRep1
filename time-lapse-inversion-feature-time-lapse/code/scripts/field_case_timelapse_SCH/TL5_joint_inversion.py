...
ttData = pg.DataContainer("rst_filtered_t1.data", "g s")
ttData2 = pg.DataContainer("rst_filtered_t2.data", "g s")
ttData3 = pg.DataContainer("rst_filtered_t3.data", "g s")
...
np.savez("TL_joint_inversion_SCH_lam%s_zW%s_delta%s_gam%s_eps%s_it%s_new_gs.npz" % (lam,zW,delta,gam,eps,maxIter), vel=np.array(velest),
         rho=np.array(rhoest), fa=fae, fi=fie, fw=fwe, fr=fre, mask=array_mask)

fit = [inv.getChi2(),inv.relrms()]
np.savetxt("fit_lam%s_zW%s_delta%s_eps%s_it%s_new_gs.txt" % (lam,zW,delta,eps,maxIter), fit)
